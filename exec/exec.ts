...
function parseCommand(command: string): { file: string; args: string[] } | null {
  // Simple shell-like parsing: split by spaces, respecting quoted strings
  const tokens: string[] = [];
  let current = "";
  let inQuotes = false;
  let quoteChar = "";
...
async function handleExec(params: { msg: any; command: string; lifecycle: GenerationContext }) {
