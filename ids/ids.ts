...
class IdsPlugin extends Plugin {
  ...
  private async getUserInfo(client: TelegramClient, user: unknown, userId: number, msg: MessageContext): Promise<UserInfo> {
    if (!user) {
      try { user = await client.getChat(userId); } catch (e: unknown) { logger.debug('[ids] getChat fallback failed:', e) }
    }
    ...
    try {
      ...
      const fullUser = (full as { users?: Array<any> }).users?.find((candidate: any) => Number(candidate.id) === userId)
        || (full as { users?: Array<any> }).users?.[0];
      if (fullUser) {
        info.user = fullUser;
  ...
  private async parseTarget(client: TelegramClient, target: string) {
    if (target.startsWith("@")) {
      try {
        ...
        // 收藏/NFT 用户名 getChat 可能解析失败，回退到用户名解析接口
